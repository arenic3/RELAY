...
CREATE TABLE IF NOT EXISTS messages(id INT AUTO_INCREMENT, message VARCHAR(255), sender_id INT, recipient_id INT, primary key(id), FOREIGN KEY (sender_id) REFERENCES users(id), FOREIGN KEY (recipient_id) REFERENCES users(id), timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);
